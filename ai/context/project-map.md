...
- 資料庫：Supabase Postgres，啟用 RLS；schema 定義於 `supabase/migrations/`。核心表：
  ...
  情境；RLS 邊界比照 `business_hours`）。Supabase Storage 另有 `store-assets` public bucket
  （TASK-029，存放 Logo／封面圖，公開讀取、只有 `is_admin()` 可寫入/更新/刪除，本專案首次
  引入檔案上傳）。顧客端（anon）對
...
- 測試：Vitest。`npm test` 為預設單元/煙霧測試（不連線真實服務）；
  ...
  `npm run test:business-hours` 是另外對真實 Supabase 專案跑的整合測試，各自獨立（見
  `vitest.integration.config.ts`／`vitest.booking.config.ts`／
  `vitest.admin-booking.config.ts`／`vitest.business-hours.config.ts`，刻意不用萬用字元
  互相撿到對方的測試檔），需要 `.env.local` 的 Supabase 設定；`test:rls`／
  `test:admin-booking`／`test:business-hours` 額外需要已 seed 的設計師帳密，`test:booking`
...
| `lib/store-settings.ts` | 商店基本資料（`store_settings`）的資料存取 | TASK-029 建立讀取函式（`getStoreSettings`），放在 `lib/` 頂層而非 `lib/admin/`——顧客前台（TASK-032）與後台（TASK-030／TASK-031）皆會讀取，比照既有「`app/admin/*` 可 import `lib/booking/*`，但顧客前台不會 import `lib/admin/*`」的既有分層方向，避免顧客前台反向依賴後台模組；寫入／圖片上傳函式留給 TASK-030／TASK-031 |
| `app/login/` | 設計師登入頁 | |
