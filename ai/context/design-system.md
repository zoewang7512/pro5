...
| Sidebar | 已完成 | 預設（使用中／停用／hover 狀態），登出項目渲染既有 `LogoutButton`；底部個人資料區塊（大頭貼＋顯示名稱，`profileName`／`profileAvatarUrl` 選填 props，未傳入時不渲染） | grey/primary 色、typography.subtitle2；個人資料區塊另用 MUI `Avatar` | MUI 沒有對應單一元件，比照 `components/ui/Nav.tsx` 用 `Box`／`List`／`ListItemButton` 拼出：`components/ui/Sidebar.tsx`；個人資料區塊直接用 MUI `Avatar` | `/admin` 後台頁面 | TASK-014；個人資料區塊 TASK-038 |
